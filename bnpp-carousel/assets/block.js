/**
 * BNPP Carousel Block - Simple Version
 */
...
(function() {
		...
		edit: function(props) {
			...
			return el('div', null,
						...
						slides.map(function(slide, idx) {
							...
							return el('div', {
								...
								el('div', {
									...
									el('small', {
										style: { fontSize: '11px', color: '#999', display: 'block', marginBottom: '12px' }
									}, slide.title.length + ' / 70'),
									el('p', {
										style: { margin: '0', fontSize: '14px', color: '#666', lineHeight: '1.5' }
									}, slide.description),
									el('small', {
										style: { fontSize: '11px', color: '#999', display: 'block', marginTop: '4px' }
				...
				wp.blockEditor && wp.blockEditor.InspectorControls ? el(wp.blockEditor.InspectorControls, null,
					...
					// TITLE
					el('div', { style: { padding: '15px', borderBottom: '1px solid #e0e0e0' } },
						el('label', { style: { display: 'block', marginBottom: '8px', fontSize: '12px', fontWeight: '600' } }, 'Title'),
						el('input', {
							type: 'text',
							value: current.title,
					...
					// DESCRIPTION
					el('div', { style: { padding: '15px', borderBottom: '1px solid #e0e0e0' } },
						el('label', { style: { display: 'block', marginBottom: '8px', fontSize: '12px', fontWeight: '600' } }, 'Description'),
						el('textarea', {
					...
					el('div', { style: { padding: '15px' } },
						el('h3', { style: { margin: '0 0 10px 0', fontSize: '12px', fontWeight: '600' } }, 'Carousel'),
						...
						attr.enableAutoPlay ? el('div', { style: { marginTop: '10px' } },
							el('label', { style: { display: 'block', marginBottom: '5px', fontSize: '11px', fontWeight: '600' } }, 'Duration (sec)'),
							el('input', { type: 'number', value: attr.autoPlayDuration || 4, onChange: function(e) { setAttr({ autoPlayDuration: parseInt(e.target.value) || 4 }); }, min: 1, max: 30, style: { width: '100%', padding: '6px 8px', border: '1px solid #ccc', borderRadius: '3px', fontSize: '12px', boxSizing: 'border-box' } })
						) : null
					)
				) : null
			);
	...
	function init() {
		document.querySelectorAll('.bnpp-carousel-wrapper').forEach(function(w) {
			...
			var idx = 0;
			var show = function(n) {
				s.forEach(function(x) { x.classList.remove('active'); });
			...
			show(0);
			t.forEach(function(x, i) { x.addEventListener('click', function() { show(i); }); });
		});
	}
